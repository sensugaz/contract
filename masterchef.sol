...
// MasterChef is the master of DEP. He can make DEP and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DEP is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DEPs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DEPs distribution occurs.
        uint256 accTokenPerShare;   // Accumulated DEPs per share, times 1e12. See below.
    ...
    // The block number when DEP mining starts.
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(tokenAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(tokenAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(tokenAddress).transfer(_to, _amount);
        }
